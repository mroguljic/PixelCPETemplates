...
//! Add cross talk and RD53 readout chip simulation
...
int main(int argc, char *argv[])
{
    ...
    float pixin[TXSIZE][TYSIZE], pixix[TXSIZE][TYSIZE];
        static bool fpix, noqbin4;
    bool ydouble[TYSIZE], xdouble[TXSIZE];
    int mrow = TXSIZE, mcol = TYSIZE;
    static float thick, xsize, ysize, noise, zcen, xcmssw, ycmssw, sxcmssw, sycmssw, gain_frac, q100_frac, common_frac, readout_noise, qscale;
    ...
    static int ndata, nfile, neh, nevent, ID, nbad, non_linear, icol, irow, irowc, ndcol, layer, nclust, run2; 
	...
	const float thrseed = 1000.;
	const float cotamax = 0.27;
    ...
    const double qperTOT = 600; // e- per TOT => step optimized for 80MHz clock=12.5ns
    const int nbitsTOT = 4; // fixed and carved in stone?
    const int tot_max = pow(2, nbitsTOT)-1;
    int tot = 0;
    
    const double qperlow = 1500.;
    const double qperhigh = 6000.;
    const double qkink = 12000.;
    const double qsplitmax = qkink+8.*qperhigh;
    
	float xtfrac = 0.1;
	static float m11, m12, m21, m22, minv11, minv12, minv21, minv22;
...
//	int random(void);
	float cluster[TXSIZE][TYSIZE], clust[TXSIZE][TYSIZE], clustx[TXSIZE][TYSIZE], rclust[TXSIZE][TYSIZE], clusterC[TXSIZE][TYSIZE];
   ...
   long deltas, deltaus;
   double deltat;
   std::string templates_dir = "../templates_32600/";		
	...
	fscanf(ifp,"%d %d %f %f %f %f %f %f %f %d %d", &ndata, &nfile, &noise, &q100, &q101, &q100_frac, &common_frac, &gain_frac, &readout_noise, &non_linear, &run2);
	fclose(ifp);
	printf("data file %d, mc file %d noise = %f, threshold0 = %f, threshold1 = %f, rms threshold frac = %f, common_frac = %f, gain fraction = %f, readout noise = %f, nonlinear_resp = %d, run2 = %d \n", ndata, nfile, noise, q100, q101, q100_frac, common_frac, gain_frac, readout_noise, non_linear, run2);
    ...
    if(run2 == 1) {
        sprintf(infile,"../template_events_1R2/template_events_d%5.5d.out",nfile);
    } else {
		sprintf(infile, "%s/template_events_d%5.5d.out", templates_dir.c_str(), nfile);        
    }
...
// Read-in a header string first and print it    
	...
	hp[41] = new TH1F("h122","npix; npix",50,0.5,50.5);
...
// Make some profile Histograms
	...
	pp[53] = new TProfile("npixvseta","npix; |#eta|",28,0.,2.8," "); 
...
// Set style for the the profile histograms	
   ...
   gtempl.pushfile(ID,thePixelGenErr_,templates_dir);	
	...
	printf("enter cross talk fraction \n");
	scanf("%f", &xtfrac);
	
	printf("cross talk fraction = %f\n", xtfrac);
	
        
// Initialize the charge sharing and inverse charge sharing matrices
   ...
   minv11 = m11/(1. - 2*xtfrac);
   minv22 = minv11;
   minv12 = -xtfrac/(1. - 2*xtfrac);
   minv21 = minv12;       
...
// Add x-talk starting at odd or even rows [randomly chosen]
		 for(j=irow; j<TXSIZE-1; j += 2) {
		    for(i=0; i<TYSIZE; ++i) {
		       pixix[j][i] = pixin[j][i]*m11 +  pixin[j+1][i]*m12;
		       pixix[j+1][i] = pixin[j][i]*m21 +  pixin[j+1][i]*m22;
             }
         }
       qpixels = 0.;
       double npix = 0.;
						...
						case 3:
						   {
						      if(qin < qkink) {
                                 tot = int((qin)/qperlow);
                                 double step = qperlow/2;
                                 signal = (qperlow*tot)+step;
                              } else {
                                 if(qin >= qsplitmax) {
                                    signal = qsplitmax;
                                  } else {
                                    tot = int((qin-qkink)/qperhigh);
                                    double step = qperhigh/2;
                                    signal = (qperhigh*tot)+step+qkink;
                                  }
                               }
                            }
					 }	 
					 clustx[TXSIZE-1-j][TYSIZE-1-i] = (1.+vgauss[0]*common_frac)*signal;
				 }
			 }
		 }
// Remove cross talk [row and column numbers are now flipped, use irowc to pick the pairings]

		 for(j=irowc; j<TXSIZE-1; j += 2) {
		    for(i=0; i<TYSIZE; ++i) {
		       clust[j][i] = clustx[j][i]*minv11 + clustx[j+1][i]*minv12;
		       clust[j+1][i] = clustx[j][i]*minv21 + clustx[j+1][i]*minv22;
             }
         }      		 
...
// keep only high pt clusters
        if(fabs(cotalpha) > cotamax) continue;
...
// Do the template analysis on the cluster 
       ...
       SiPixelTemplateReco::ClusMatrix clusterPayloadC{&clusterC[0][0], xdouble, ydouble, mrow,mcol};
	   locBx = 1.;
	   ...
	   if(ierr != 0) {
	      ...
	      printf("reconstruction failed with error %d \n", ierr);
		    ...
		    if(sigmax <= 0.f || sigmay <=0.f) continue;
		    if(isnan(sigmax) || isnan(sigmay)) continue;
            qb = qbin;
...
//  Create an output filename for this run 
   ...
   sprintf(outfile0,"%s/pixel_histos%5.5d.pdf[", templates_dir.c_str(),nfile);
   sprintf(outfile1,"%s/pixel_histos%5.5d.pdf", templates_dir.c_str(),nfile);
   sprintf(outfile2,"%s/pixel_histos%5.5d.pdf]", templates_dir.c_str(),nfile);
