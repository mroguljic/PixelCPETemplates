...
/* Subroutine */ int propag(float thick, float xsize, float ysize, float impwdth, float implenn, float implenp, float temp, float flux[2], float rhe, float rhh, int ehole, int neh, vect_or_f xeh[2][NEHSTORE], int indeh[2][NEHSTORE])
{
   /* Initialized data */
   ...
   static float qe[2], diflen, mu, mu2, difcon, rnorm, trptim[2];
   float timexp, dnear, tnear, rvec[3];
   static int nindeh[2];
...
/* Subroutine */ int detect(float xsize, float ysize, float thick, float impwdth, float implenn, float implenp, int ehole, vect_or_f xeh[2][NEHSTORE], int neh, float pixel[2][TXSIZE][TYSIZE])
{
   /* System generated locals */
   int i__1;
   ...
   for (ieh = 0; ieh < neh; ++ieh) {
      ...
      if (ix > -1 && ix < TXSIZE && iy > -1 && iy < TYSIZE) {
         if (boundary(&xeh[ehole][ieh], ehole, ehole, thick, xsize, ysize, impwdth, implenn, implenp) < 0.) {
            ++pixel[0][ix][iy];
         }
      }
   }
   ...
   for (j=0; j<2; ++j) {        
      for (ieh = 0; ieh < neh; ++ieh) {   
         ...
         if (ix > -1 && ix < TXSIZE && iy > -1 && iy < TYSIZE) { 
            if (boundary(&xeh[ehole][ieh], j, ehole, thick, xsize, ysize, impwdth, implenn, implenp) >= 0.) { 
               xp.v = xeh[j][ieh].v;
...
/* Subroutine */ int pixinit(float *pimom, float *thick, float *xsize, float *ysize, float *impwdth, float *implenn, float *implenp, float *temp, float flux[2], float *rhe, float *rhh, int *ehole, int *new_drde, int *filebase)
{
    ...
    ifp = fopen("pixel3d.init", "r");
    if (ifp==NULL) {
		printf("no pixel3d.init initialization file found/n");
		return 0;
    }
    ...
    fscanf(ifp,"%f %f %f %f %f %f %f %f %f %f %f %d %d %d %d %d", thick, xsize, ysize, impwdth, implenn, implenp, temp, &flux[0], &flux[1], rhe, rhh, ehole, new_drde, &npixx, &npixy, &npixz);
	if((*ehole) !=0) { *ehole = 1;}
    
    printf("thickness = %f, x/y sizes = %f/%f, implant 1/2 width/lenn/lenp = %f/%f/%f, temp = %f, flux_e = %f, flux_h = %f, rhe = %f, rhh = %f, ehole = %d, new_drde = %d \n", 
	       *thick, *xsize, *ysize, *impwdth, *implenn, *implenp, *temp, flux[0], flux[1], *rhe, *rhh, *ehole, *new_drde);
...
/* Read in weighting potential file */
    ...
    if (ifp==NULL) {
		printf("no wgt_pot3d.init initialization file found/n");
		return 0;
    }
