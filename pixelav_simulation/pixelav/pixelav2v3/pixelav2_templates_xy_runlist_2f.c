...
/* merge ppc and i386 codebases into a common file */
/* Add ehole flag to select electron or hole signal calculation */
/* Split initialization between global and local files */
/* read runlist and run selected ones */
...
/* Define the maxumum number of template points */
...
/* Define array to help with bounds checking */
    ...
    static float bfield_y;
    static float bfield_x;
    static char header[80];
/* function prototypes */

#include "pixelav2_prototypes.h"

    int runinit(int *, int *, float *, float *, int *);
...
/* Main program */ int main(int argc, char *argv[])
{
    ...
    /* Local variables */
    static float vect[6];
	static float lenx[TEMPMAX], leny[TEMPMAX];
    static float thick, xsize, ysize, temp, flux[2], rhe, rhh;
    static int i__, indeh[2][NEHSTORE]	/* was [2][300000] */;
    static int nto2in, lux, initseed, ivec[25];
    static float pixel[2][TXSIZE][TYSIZE];
    static int ntotin, neh, runsiz;
    static vect_or_f xeh[2][NEHSTORE]	/* was [4][2][300000] */;
    time_t now;
    struct tm *nows;
    int sec, min, hour, yday, j, k;
    static int fileind, filebase, fileoff[TEMPMAX], runsize[TEMPMAX], irun, ievent, nrun, procid, new_drde, ehole, nrmax, frun;
    static float rvec[2], pimom, xoffset, yoffset, locdir[3], cotalpha, cotbeta;
...
/*  Define the detector parameters from the global initialization files */
    ...
    if(pixinit(&pimom, &thick, &xsize, &ysize, &temp, flux, &rhe, &rhh, &ehole, &new_drde, &filebase) == 0) {return 0;}
    
    if(pimom < 1.1) {pimom = 45.;}

/*  Define the run parameters from the local run initialization file */
	
    runinit(fileoff, runsize, lenx, leny, &nrmax);
	
/*  Test legality of run specifiers against number of runs read */

	if(frun >= nrmax) {printf("frun %d is larger than runlist size, quit \n", frun); return 0;}
	if(nrun > (nrmax-frun)) {printf("nrun %d will exceed runlist size, quit \n", nrun); return 0;}
...
/* read-in 25 ints and restore generator to previous state */
    ...
    while(irun < nrun) {
	   ...
	   clusxlen = lenx[frun+irun]*xsize;
	   cotbeta = clusxlen/thick;
	   clusylen = leny[frun+irun]*ysize;
...
/*  copy  header to the output file on first event */
	   ...
	   runsiz = runsize[frun+irun];
	   
/* Generate the requested number of events for this direction */
...
/* Generate initial position and direction of the track */
         ...
         vect[3] = pimom*locdir[0];
         vect[4] = pimom*locdir[1];
         vect[5] = pimom*locdir[2];
...
/*  Propagate the e's and h's */
             ...
             propag(thick, xsize, ysize, temp, flux, rhe, rhh, ehole, neh, xeh, indeh);

/*  Count e's and h's on various pixels */

             detect(xsize, ysize, thick, ehole, xeh, neh, pixel);
...
/*  Determine current time */
		  ...
		  if(ievent < 10) {printf("day %d at %02d:%02d:%02d, run %d, event %d, number of e-h pairs = %d\n", yday, hour, min, sec, irun, ievent, neh);}
...
/* Subroutine */ int runinit(int *fileoff, int *runsize, float *lenx, float *leny, int *nrmax)
{
	...
	/* ************************************************************************ */
	/* * This routine initializes local run parameters from file runlist.init * */
	/* * Parameters: fileoff - local offset for the first output file #       * */
	/* *             runsize - the # events in each run                       * */
	/* *                lenx - the cluster x-length                           * */
	/* *                       (signed so that +len is along +x axis)         * */
	/* *                leny - the cluster y-length                           * */
	/* *                       (signed so that +len is along +y axis)         * */
	/* *               nrmax - length of run list                             * */
	/* ************************************************************************ */
    ...
    ifp = fopen("runlist.init", "r");
    if (ifp==NULL) {
		printf("no runlist.init initialization file found/n");
		return 0;
    }
	...
	irun = 0;
    while(fscanf(ifp,"%d %d %f %f", &fileoff[irun], &runsize[irun], &lenx[irun], &leny[irun]) != EOF) {++irun; if(irun == TEMPMAX) break;}
	fclose(ifp);
