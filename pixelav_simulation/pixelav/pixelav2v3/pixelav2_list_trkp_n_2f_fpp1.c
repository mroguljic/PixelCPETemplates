...
/* For phase 1 forward pixels only */
...
/* Define array to help with bounds checking */
    ...
    static float bfield_z, bfield_x, bfield_y;
...
/* Main program */ int main(int argc, char *argv[])
{
	...
	static float clusxlen, clusylen, locBx, locBz;
...
/* Now read-in track angles and momenta to process */
	...
	if(fileind < 10000) {
        sprintf(seedfile,"seedfile%4.4d",fileind);
    } else {
...
/* read-in 25 ints and restore generator to previous state */
    ...
    if(fileind < 10000) {
        sprintf(outfile,"template_events_d%4.4d.out",fileind);
    } else {
...
/*  Set Bfield direction for this event */
	   ...
	   bfield.f[1] = locBx*bfield_y;
	   bfield.f[2] = locBz*bfield_z;
	   bfield.f[0] = locBz*locBx*bfield_x;
