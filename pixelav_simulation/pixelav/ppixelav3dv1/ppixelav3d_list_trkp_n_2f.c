...
#include "ppixelav3d_prototypes.h"
...
/* Main program */ int main(int argc, char *argv[])
{
    ...
    static float thick, xsize, ysize, impwdth, implenn, implenp, temp, flux[2], rhe, rhh, peaktim, samptim, stimstp;    
    ...
    if(pixinit(&pimom, &thick, &xsize, &ysize, &impwdth, &implenn, &implenp, &temp, flux, &rhe, &rhh, &peaktim, &samptim, &ehole, &new_drde, &filebase) == 0) {return 0;}	 
...
/*  Propagate the e's and h's (no signal after the sample time) */
			 ...
			 propag(thick, xsize, ysize, impwdth, implenn, implenp, temp, flux, rhe, rhh, samptim, stimstp, ehole, neh, xeh, indeh, xhisteh);
				
/*  Count e's and h's on various pixels */

			 detect(xsize, ysize, thick, impwdth, implenn, implenp, ehole, xhisteh, neh, crrcresp, pixel, pixhist);
