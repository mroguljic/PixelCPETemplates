/* Subroutine */ int deposit(float *vect, float thick, int new_drde, int nmax, vect_or_f xeh[2][NEHSTORE], int *neh)
{
	...
	/* *                         VECT[3-5] are the momentum in GeV/c        * */
...
/* Subroutine */ int propag(float thick, float xsize, float ysize, float impwdth, float implenn, float implenp, float temp, float flux[2], float rhe, float rhh, float max_drift_time, float stimstp, int ehole, int neh, vect_or_f xeh[2][NEHSTORE], int indeh[2][NEHSTORE], vect_or_f xhisteh[NCRRC][2][NEHSTORE])
{
	/* Initialized data */
	...
	float timexp, dnear, tnear, zproj, rvec[3];
	static int nindeh[2];
	...
	for (ieh = 0; ieh < neh; ++ieh) {
      dnear = boundary(&xeh[ehole][ieh], ehole, ehole, thick, xsize, ysize, impwdth, implenn, implenp);
      if((ieh/Nscale)*Nscale == ieh && dnear > 0.) {
			...
			for (ind = 0; ind < nind; ++ind) {
				...
				while(dx2 > deltmax) {
				   ...
				   rnorm = 1. + rhbf2[jeh] * mu2;
                    ...
                    dnear = boundary(&x[0], jeh, ehole, thick, xsize, ysize, impwdth, implenn, implenp);
                    tnear = dnear/sqrt(v[0].f[0]*v[0].f[0]+v[0].f[1]*v[0].f[1]+v[0].f[2]*v[0].f[2]);
                    if(timeh[jeh][ieh] > tnear) timeh[jeh][ieh] = tnear;
                ...
                if(boundary(&xeh[jeh][ieh], jeh, ehole, thick, xsize, ysize, impwdth, implenn, implenp) <= 0.) {
					indeh[jeh][ind] = -1;
					xhisteh[itimstp[jeh][ieh]-1][jeh][ieh].v = xeh[jeh][ieh].v;
					for(i__ = itimstp[jeh][ieh]; i__ < NCRRC; ++i__) { xhisteh[i__][jeh][ieh].v = xeh[jeh][ieh].v;}
				}
...
double dsdlne(float ppion, float einlog)
{
    ...
    static float dslog;
    static float engy[1251], gambet[60], xs[1251][60];
...
/* Make sure file is available */
    ...
    extfp = fopen("SIRUTH2.SPR", "r");
    if (extfp==NULL) {
       printf("cant find SIRUTH2.SPR");
       return 1;
    }
...
/* Subroutine */ int detect(float xsize, float ysize, float thick, float impwdth, float implenn, float implenp, int ehole, vect_or_f xhisteh[NCRRC][2][NEHSTORE], int neh, float respfcn[NCRRC], float pixel[TXSIZE][TYSIZE], float pixhist[NCRRC+1][TXSIZE][TYSIZE])
{
    /* System generated locals */
    int i__1;
...
/*  Calculate the coordinates of each electron/hole and see if it was collected */
       ...
       for (ieh = 0; ieh < neh; ++ieh) {
           if(boundary(&xhisteh[jhist][ehole][ieh], ehole, ehole, thick, xsize, ysize, impwdth, implenn, implenp) < 0.) {
...
/* Subroutine */ float boundary(vect_or_f *xp, int jeh, int ehole, float thick, float xsize, float ysize, float impwdth, float implenn, float implenp)
{
   /* Initialized data */
   ...
   static unsigned int ix[3][4];
   static vect_or_f igrid, vzero, vhalf, pixel, pgrid, vfloor, xshift;
   static vect_or_f x, ax;
   static float xcorner, ycorner;
   float zpost;
   
   
   /* ***************************************************************** */
   /* * This routine calculates the distance from xp to the appropriate * */
   /* * boundary.  It returns -1. if the carrier has been collected.    * */
   /* * Parameters: XP.f[4] - position of the particle (in microns)     * */
   /* *             jeh   - the electron/hole index of the carrier      * */
   /* *             ehole - 0/1: the detector collects e or h           * */
   /* *             thick - the sensor size in z in um                  * */
   /* *             xsize - the sensor size in x in um                  * */
   /* *             ysize - the sensor size in y in um                  * */
   /* *             impwdth - the 1/2 width of the 3d implant or depth  * */
   /* *                       "overshoot" for planar sensors            * */
   /* *             implenn - the length of the 3d n+ implant or <=0 to * */
   /* *                       select planar sensors                     * */
   /* *             implenp - the length of the 3d p+ implant           * */  
   /* ***************************************************************** */
   ...
   /* Check to see if carriers are outside the sensor */
   if(x.f[2] <=0. || x.f[2] >= thick) return -1.;
   
   /* If implenn is <=0, do everything for planar sensor */
   
   if(implenn <=0.) {
      if(jeh == 0) {
         return (thick+impwdth - x.f[2]);
      } else {
         return (x.f[2] + impwdth);
      }
   } else {
      
      /* First consider the cases when the carriers are in or above/below the implants */
      zpost = thick - implenn;
      
      if(ax.f[0] < impwdth && ax.f[1] < impwdth){
         if(ehole == 0) {
            if(ax.f[2] > zpost) {
               return -1.;
            } else {
               return sqrt(ax.f[0]*ax.f[0]+ax.f[1]*ax.f[1]+(zpost+impwdth - ax.f[2])*(zpost+impwdth - ax.f[2]));
            }
         } else {
            if(ax.f[2] < implenp) {
               return -1.;
            } else {
               return sqrt(ax.f[0]*ax.f[0]+ax.f[1]*ax.f[1]+(ax.f[2]-implenp+impwdth)*(ax.f[2]-implenp+impwdth));
            }
         }
      }
      if((xcorner-ax.f[0]) < impwdth && (ycorner-ax.f[1]) < impwdth){
         if(ehole == 0) {
            if(ax.f[2] < implenp) {
               return -1.f;
            } else {
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1])
                           +(ax.f[2]-implenp+impwdth)*(ax.f[2]-implenp+impwdth));
            }
         } else {
            if(ax.f[2] > zpost) {
               return -1.f;
               ...
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1])
                           +(zpost+impwdth - ax.f[2])*(zpost+impwdth - ax.f[2]));
            }
         }
      }
      
      /* OK, the carriers are in the pixel bulk, calculate the distance to the appropriate implant */
      
      if(ehole == 0) {
         if(jeh == 0) {
            /* electrons drift to the central n+ implant */
            if(ax.f[2] > zpost) {
               /* return just the transverse distance to (0,0) */
               return sqrt(ax.f[0]*ax.f[0]+ax.f[1]*ax.f[1]);
            } else {
               return sqrt(ax.f[0]*ax.f[0]+ax.f[1]*ax.f[1]+(zpost+impwdth - ax.f[2])*(zpost+impwdth - ax.f[2]));
            }
         } else {
            /* holes drift to the corner p+ implant */
            if(ax.f[2] < implenp) {
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1]));
            } else {
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1])
         ...
         if(jeh == 1) {
            /* holes drift to the central p+ implant */
            if(ax.f[2] < implenp) {
               /* return just the transverse distance to (0,0) */
               return sqrt(ax.f[0]*ax.f[0]+ax.f[1]*ax.f[1]);
            } else {
            ...
            /* electrons drift to the corner n+ implant */
            if(ax.f[2] > zpost) {
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1]));
            } else {
               return sqrt((xcorner-ax.f[0])*(xcorner-ax.f[0])+(ycorner-ax.f[1])*(ycorner-ax.f[1])
...
/* Subroutine */ int pixinit(float *pimom, float *thick, float *xsize, float *ysize, float *impwdth, float *implenn, float *implenp, float *temp, float flux[2], float *rhe, float *rhh, float *peaktim, float *samptim, int *ehole, int *new_drde, int *filebase)
{
	...
	ifp = fopen("ppixel3d.init", "r");
	if (ifp==NULL) {
      printf("no ppixel3d.init initialization file found/n");
      return 0;
	}
	...
	fscanf(ifp,"%f %f %f %f %f %f %f %f %f %f %f %f %f %d %d %d %d %d", thick, xsize, ysize, impwdth, implenn, implenp, temp, &flux[0], &flux[1], rhe, rhh, peaktim, samptim, ehole, new_drde, &npixx, &npixy, &npixz);
	if((*ehole) !=0) { *ehole = 1;}
	
	printf("thickness = %f, x/y sizes = %f/%f, implant wdth/nlen/plen = %f/%f/%f, temp = %f, flux_e = %f, flux_h = %f, rhe = %f, rhh = %f, peaktim = %f, samptim = %f, ehole = %d, new_drde = %d \n", 
	       *thick, *xsize, *ysize, *impwdth, *implenn, *implenp, *temp, flux[0], flux[1], *rhe, *rhh, *peaktim, *samptim, *ehole, *new_drde);
