...
//  V10.30 - Make Dtype available via a method
//  V10.30 - Add Dtype=6 to describe an FPix 3d sensor
...
struct SiPixelTemplateStore { //!< template storage structure
   ...
   float cotbetaX[5];
   float cotalphaX[29];
   SiPixelTemplateEntry enty[61];     //!< 60 Barrel y templates spanning cluster lengths from 0px to +18px [28 entries for fpix]
...
//   ${dir}template_summary_zp${filenum}.out
   ...
   int Dtype() {return Dtype_;}        //!< return subdetector code
