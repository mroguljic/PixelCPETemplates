//
//  SiPixelTemplate2D.cc  Version 2.70
//
...
//  Read the 2D template entries as floats [they are formatted that way] and cast to short ints
	      ...
	      return false;
            ...
            >> theCurrentTemp.entry[iy][jx].cce  >> theCurrentTemp.entry[iy][jx].spare[1];
...
bool SiPixelTemplate2D::interpolate(int id, float cotalpha, float cotbeta, float locBz, float locBx)
{
   ...
   cce_ = entry00_->cce
   +adcota_*(entry01_->cce - entry00_->cce)
   +adcotb_*(entry10_->cce - entry00_->cce);
