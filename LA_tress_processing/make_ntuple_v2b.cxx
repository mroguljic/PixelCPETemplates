...
//  Create an input filename for this run 
	...
	sprintf(ntupfile,"pixelav_events_n%5.5d.root",nfile);
...
// Calculate row and column numbers (remember that we will merge the first two rows/columns);
			...
			pixinfo_.x[npix] = (2.5+j)*xsize*0.0001;
			pixinfo_.y[npix] = (2.5+i)*ysize*0.0001;
			++npix;
			if(npix == 100) break;
		}
		pixinfo_.npix = npix;
...
//  Create an output filename for this run 
   ...
   sprintf(outfile0,"pixel_histos%5.5d.ps[",nfile);
   sprintf(outfile1,"pixel_histos%5.5d.ps",nfile);
   sprintf(outfile2,"pixel_histos%5.5d.ps]",nfile);
   c2.Print(outfile0);
   for(i=0; i<24; ++i) {
     hp[i]->Draw();
     c2.Print(outfile1);
