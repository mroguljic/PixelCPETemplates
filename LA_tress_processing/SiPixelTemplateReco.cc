...
#else
      if((sigmaQ <= 0.) || (mpv <= 0.) || (kappa < 0.009) || (kappa > 10.)) LOGERROR("SiPixelTemplateReco") << "SiPixelTemplateReco::Vavilov parameters mpv/sigmaQ/kappa = " << mpv << "/" << sigmaQ << "/" << kappa << std::endl;
//      assert((sigmaQ > 0.) && (mpv > 0.) && (kappa > 0.009) && (kappa < 10.));
#endif
